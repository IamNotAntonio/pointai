...
export default function ChatPage() {
  return (
    <Suspense fallback={<div style={{ height:'100vh', display:'flex', alignItems:'center', justifyContent:'center', color:'var(--text-4)' }}>Carregando…</div>}>
      <Dashboard />
    </Suspense>
  )
}
...
function Dashboard() {
  ...
  useEffect(() => {
    async function carregarPerfil() {
      ...
      if (p) {
        ...
        setMaterias(lista)
        // Resolve initial matéria from URL param. Falls back to Chat Geral.
        const fromUrl = materiaParam && (materiaParam === '__geral__' || lista.includes(materiaParam))
          ? materiaParam
          : '__geral__'
        setMateriaAtiva(fromUrl)
      }
    }
    carregarPerfil()
  ...
  // Sync URL ?materia= when the user switches matéria from sidebar.
  useEffect(() => {
    if (!materiaAtiva) return
    const desired = materiaAtiva === '__geral__' ? null : materiaAtiva
    if (desired === materiaParam) return
    const params = new URLSearchParams(searchParams.toString())
    if (desired) params.set('materia', desired)
    else params.delete('materia')
    const qs = params.toString()
    router.replace(qs ? `${pathname}?${qs}` : pathname, { scroll: false })
  }, [materiaAtiva])

  // React to URL changes (e.g. user clicks a different matéria in the sidebar).
  useEffect(() => {
    if (!materias.length && materiaParam !== '__geral__') return
    const desired = materiaParam && (materiaParam === '__geral__' || materias.includes(materiaParam))
      ? materiaParam
      : '__geral__'
    if (desired !== materiaAtiva) {
      setMateriaAtiva(desired)
      setTopicoAtivo(null)
      setMensagens([])
      setImagem(null)
    }
  }, [materiaParam, materias])
  ...
  async function _stream(histMensagens, pdfRequested, imagemEnviada) {
    ...
    // Track "última conversa" for the dashboard home Continuar card.
    try {
